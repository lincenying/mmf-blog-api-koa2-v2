...
module.exports = function proxy(app, api, config, options) {
    ...
    return async function(ctx, next) {
        ...
        const preg = /__data__$/
        const isDebug = config.allowShowApi && preg.test(ctx.req.url)
        ...
        Object.assign(ctx, {
            proxy(opt, config) {
                ...
                // 装载头信息的容器
                const headerObj = config.headers

                const reqsParam = Object.keys(opt)
                const proxyProto = '__proxyname__'

                const reqs = reqsParam.map(proxyName => {
                    // 分析当前proxy请求的URL
                    const realReq = setRequest(ctx, opt[proxyName])

                    // 请求request的最终配置
                    const requestOpt = Object.assign(
                        {},
                        options,
                        {
                            uri: realReq.uri,
                ...
                return new Promise(resolve => {
                    Promise.all(reqs).then(data => {
                        ...
                        data.forEach(response => {
                            if (!response) return
                            const proxyName = response[proxyProto]
            ...
            fetch(url, config) {
                ...
                // 获取头信息
                const realReq = setRequest(ctx, url)
                // 请求request的最终配置
                const requestOpt = Object.assign(
                    {},
                    options,
    ...
    function setRequest(ctx, path) {
        ...
        const urlObj = analyUrl(ctx, path)
        const url = addQuery(urlObj.url, query)
        const uriObj = getUri(url)
    ...
    function addQuery(url, query) {
        ...
        const queStr = querystring.stringify(urlQue)
     ...
     */
    function getUri(url) {
        const uriObj = url_opera.parse(url)
